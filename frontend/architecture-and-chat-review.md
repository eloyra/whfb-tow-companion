...
>
> **Recheck 2026-07-03**: Frontend-alone review pass performed. Findings marked ✅ RESOLVED below were verified fixed against current code (`ChatInterface.tsx@HEAD`, `package.json@HEAD`). Do not re-apply the proposed fix for resolved items.
...
#### 3.1 FSD compliance
- **[MEDIUM] `src/entities/` is empty — no domain models exist yet.** Populate (Unit, SpecialRule, Spell, MagicItem mirroring graph schema) when graph viewer lands.
...
- **[HIGH] No abort on unmount.** `useChat` returns `stop`, but it isn't called in cleanup effect. If user navigates away mid-stream the request keeps streaming server-side. Add:
  ...
  useEffect(() => () => stop(), [stop]);
  ```
  > ✅ **RESOLVED** (`ChatInterface.tsx:38-43`): `useEffect(() => () => stop(), [stop])` now aborts the stream on unmount.
...
- **[MEDIUM] `scrollIntoView` on every `messages` change** (line 26-28) is jumpy: it interrupts user reading prior content when a new token arrives. Standard pattern: only auto-scroll when user is already near the bottom. An `isNearBottom` ref + threshold check.
  > ✅ **RESOLVED** (`ChatInterface.tsx:31-35`): `isNearBottom` threshold check (`distanceFromBottom < 100`) gates auto-scroll.
- **[MEDIUM] `ReactMarkdown` with no `rehype-sanitize`** — assistant output is rendered as markdown without sanitisation. LLM can be tricked into emitting `<script>` or `javascript:` URLs. ReactMarkdown defaults filter most XSS, but adding `rehype-sanitize` is industry standard and trivial.
  > ✅ **RESOLVED** (`ChatInterface.tsx:7`): `rehype-sanitize` now imported and applied to `ReactMarkdown`.
...
- **[MEDIUM] `pnpm` not pinned via `packageManager`** field in `package.json`. CI / new contributors may use a wrong major. Add `"packageManager": "pnpm@<version>"`.
  > ✅ **RESOLVED** (`package.json:packageManager: "pnpm@10.33.2"`).
...
| Frontend abort on unmount | ✅ Fixed | ~~HIGH~~ resolved |
...
| Markdown XSS sanitisation | ✅ Fixed | ~~MEDIUM~~ resolved |
...
| Empty `entities/` | Placeholder | LOW |
...
4. **Add request abort handling** server-side ~~and `useEffect` cleanup `stop()` client-side~~ (✅ client-side resolved 2026-07-03).
...
9. ~~**Sanitise markdown** with `rehype-sanitize`.~~ (✅ resolved 2026-07-03).
